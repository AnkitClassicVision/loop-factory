#!/usr/bin/env python3
"""Generate manifest-r3-u5-digest-hardening.json for the loop-drive-contract job.

Round 2 passed its check and still shipped a silent-drop path: when the digest
tool fails, `body_digest` comes back empty, an absent marker reads as empty too,
and the equality test suppresses an escalation nobody ever saw. The check was
extended with scenario s7 to catch exactly that, proven red against round 2's
patch and green against a hardened variant before this round was compiled.

Round 3 rebuilds U5 from HEAD with that finding in hand. The worker never sees
round 2's patch — a fix swarm consumes review findings, not the previous
worker's code.

Edit LANES, then:  python3 build_r3.py
"""
...
WORKDIR = "/mnt/d_drive/ringer-work/loop-drive-contract-r3"
...
9. A digest that cannot be computed must never read as "already delivered". This is a
   real finding from an earlier attempt at this same fix, so do not rediscover it the
   hard way. That attempt wrote:

       body_digest=$(sha256sum "$ESCALATE_FILE" 2>>"$LOG" | awk '{{print $1}}') || return 1

   When the digest tool fails, `awk` still exits 0, so the `|| return 1` never fires
   and `body_digest` is the empty string. With no marker on disk the recorded digest
   is empty too, the equality test says "already delivered", and the function returns
   0 having sent nothing. Keying suppression on a hash creates that failure mode;
   existence-keying never had it. So: if the digest is empty, deliver rather than
   suppress (or return non-zero). Silence is the one outcome that is never acceptable
   on this path.
...
trusted. Then it runs the same scenarios against your edited file, including one that
makes every common digest tool on PATH fail and requires the alert to survive it.
Every failure names the scenario and prints the sink and marker state.

Run it BEFORE you edit, to watch the expected failures. Run it again after. Also run
`bash -n {SCRIPT}` for a syntax check.
...
for suffix, engine, model, engine_args in LANES:
    ...
    task: dict[str, object] = {
        ...
        # A passing task's worktree is deleted, so deliverables only survive as
        # the copies the check exports into WORKDIR.
        "expect_files": [f"{WORKDIR}/{key}.patch", f"{WORKDIR}/{key}.summary.md"],
        "verified": (
            "the escalation marker records a digest of the delivered body and no path "
            "swallows an alert: the baseline still reproduces the dropped same-day "
            "alert, the fixed code delivers a changed body, suppresses an identical "
            "one, appends the runner status line after a delivery, writes no marker "
            "when delivery fails, and still alerts when the digest cannot be computed"
        ),
    }
    if model:
        task["model"] = model
...
out = HERE / "manifest-r3-u5-digest-hardening.json"
