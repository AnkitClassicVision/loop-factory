...
| F3 Mailroom | Register `departments/<dept>/state/decisions_outbox.jsonl` in the shared mailroom config with `factory/mailroom_registry.py`; a missing fresh-machine config is reported loudly | deterministic | one valid mailroom watch entry exists, or the missing-config warning is recorded |
...
7. Mailroom watch registered for the department decisions outbox, with the
   shared config validated and the entry idempotent.
8. Release pinned, `current` flipped, drift check clean.
9. Escalation path proven once end-to-end (a test escalation reached the
   human-in-the-loop outbox).
10. Zero external effects: no send, write, publish, or spend occurred (shadow).
