...
def run(root:Path)->dict:
    state=root/"departments/outreach/state";specs={"outreach_state_drift":("state_reconcile.json","outreach_state_drift"),"send_class_integrity":("gate_monitor.json","send_class_integrity"),"approval_queue_aged":("queue_ager.json","approval_queue_aged")};values={}
    ...
    output={"schema":"objectives-observed/v1","ts":utcnow().isoformat(),"values":values};atomic_json(state/"objectives_observed.json",output);return output
def main()->int:
